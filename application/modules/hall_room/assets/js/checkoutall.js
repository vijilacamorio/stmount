...
function userselect(e) {
    ...
    if (k != e) {
        ...
        var myurl = baseurl + "hall_room/hallroom/customerpay/" + checkedval;
        $.ajax({
            url: myurl,
            type: "POST",
            data: {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.substr(4, 1) === "S") {
                ...
                $("#print_invoice_all").trigger('click');
                toastrSuccessMsg(data);
                $(".sidebar-mini").removeClass('sidebar-collapse');
            } else
                toastrErrorMsg(data);
...
function podataprintflist() {
    ...
    $.ajax({
        ...
        url: base + "hall_room/hallroom/smpooldetails",
        data: {
            csrf_test_name: csrf,
            pid: pb_id
...
function restaurantBill() {
    ...
    $.ajax({
        type: "POST",
        url: base + "hall_room/hallroom/restaurantDetails",
        data: {
            csrf_test_name: csrf,
            order_id: order_id
...
function hallRoomBill() {
    ...
    $.ajax({
        type: "POST",
        url: base + "hall_room/hallroom/hallDetails",
        data: {
            csrf_test_name: csrf,
            hall_id: hall_id
...
$("#previous").on("click", function() {
    window.location.href=baseurl+"hall_room/checkout-list";
