...
$(document).ready(function() {
    ...
    //         TOTAL += parseFloat($("#get_total" + s).val());
    //     }
    ...
    //     $("#discountAmount").text("-" + disamount.toFixed(2));
    //     $("#total_charge").text(final_dis.toFixed(2));
    //     $(".get_total").val(final_dis.toFixed(2));
    //     $('#totalamount').val(final_dis.toFixed(2));
    ...
    $("#from_date1-1, #to_date1-1").on("change", function () {
        debugger;
        var checkindate = $('#from_date1-1').val();
        var checkoutdate = $('#to_date1-1').val();
        ...
        var rent_amount = parseFloat($("#rent-1").val()) || 0;
        ...
        if (checkindate && checkoutdate && !isNaN(rent_amount)) {
            ...
            if (!isNaN(diff) && diff !== 0) {
                ...
                $('#rent-1').val(total_amount.toFixed(2)); 
                var all_person = 0;
                var all_child = 0;
                var all_bed = 0;
                ...
                for (var s = 1; s <= all; s++) {
                    var rentValue = parseFloat($("#amount1-1" + s).val()) || 0;
                    if (!isNaN(rentValue)) {
                        all_bed += rentValue;
                    }
                }
                ...
                for (var s = 1; s <= all; s++) {
                    var rentValue = parseFloat($("#amount2-1" + s).val()) || 0;
                    if (!isNaN(rentValue)) {
                        all_person += rentValue;
                    }
                }
                ...
                for (var s = 1; s <= all; s++) {
                    var rentValue = parseFloat($("#amount3-1" + s).val()) || 0;
                    if (!isNaN(rentValue)) {
                        all_child += rentValue;
                    }
                }
   ...
   $("#rent-1").on("change", function(){
    ...
    //     }
    //     if (cgst > 0 && sgst > 0) {
    //         cgst = ((cgst + sgst) * rent) / 100;
    //     }
    //     var total = rent + cgst + sgst + alltotal; 
    //     $("#totalamount").val(total);
    //     $("#booking_charge").text(rent);
    //     $("#cgst_charge").text(cgst);
    //     $("#sgst_charge").text(cgst);
    //     $(".cgst_charge").val(cgst);
    //     $(".sgst_charge").val(cgst);
...
function room(r,s=null){
  ...
  //  debugger;
    ...
    col += '<tr><td colspan="2"><table class="table table-borderless mb-0"><tbody><tr>'
           ...
           +'id="adults'+i+'" placeholder="No of persons"><label class="fas fa-restroom"></label></div></div></td></tr></tbody></table></td><td rowspan="3" class="text-center"><button type="button" class="btn btn-danger btn-sm rdel'+i+'" onclick="rdel('+i+')"><i class="far fa-trash-alt"></i></button>'
           +'</td></tr>';
    ...
    col += '<tr><td class="p-0"><table class="table table-borderless mb-0 bg-light"><tbody><tr><td style="display:none;" class="border-0"><input type="text" disabled class="form-control ex-room datefilter3" id="from_date2'+i+'"placeholder="yyyy/mm/dd" value=""></td><td style="display:none;" class="border-0">'
        ...
        if (checkindate && checkoutdate && !isNaN(rent_amount)) {
            ...
            if (!isNaN(diff) && diff !== 0) {
                ...
                $('#rent' + i).val(total_amount.toFixed(2)); 
                  var all_person = parseFloat($("#amount2" + i).val()) || 0;
                  var all_child = parseFloat($("#amount3" + i).val()) || 0;
                  var all_bed = parseFloat($("#amount1" + i).val()) || 0;
                  var all = $("table.room-list > tbody tr").length;
                  var baseRent = parseFloat($('#rent-1').val()) || 0;
                  var totalCGSTPercentage = parseFloat($("#cgst").val()) || 0;
                  var totalSGSTPercentage = parseFloat($("#sgst").val()) || 0;
    ...
    grandTotal=parseFloat(grandTotal-discount);
            ...
            //  $("#total_charge").text(grandTotal.toFixed(2));
             ...
             // $("#amount3").val(childprice);
              ...
              $('.get_total').val(grandTotal.toFixed(2));
              $('#totalamount').val(grandTotal.toFixed(2));
            // $("#total_charge").text(newtotal1.toFixed(2));
            // $('.get_total').val(newtotal1.toFixed(2));
            // $('#totalamount').val(newtotal1.toFixed(2));
