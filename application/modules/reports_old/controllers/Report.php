 ...
 public function nightaudit(){
	...
	//echo $this->db->last_query();	
	 //  print_r($data["tax_with_amount"]);die();
		$data['nightaudit']  = $this->report_model->nightaudit();
		...
		$today_hall=$this->report_model->nightaudit_payment_method_hall();
		$month_hall=$this->report_model->nightaudit_payment_method_hall_month();
		$year_hall =$this->report_model->nightaudit_payment_method_hall_year();
        ...
        foreach ($today_payment_month_room as $row) {
            $payment_data_resto[$row->payment_method]['month'] = $row->bill_amount;
        }

        // Fill in the data for the year
        foreach ($today_payment_year_room as $row) {
            $payment_data_resto[$row->payment_method]['year'] = $row->bill_amount;
        }
		...
		$data['nightaudit_current_month']  = $this->report_model->nightaudit_current_month();
...
$data['processed_data'] = $processed_data;
	...
	// print_r($data['occupancy_data_year']);echo "<br/>";
		...
		$payment_hall = array();
        foreach ($today_hall as $row) {
            $payment_hall[$row->paymenttype]['today'] = $row->paymentamount;
        }
        foreach ($month_hall as $row) {
            $payment_hall[$row->paymenttype]['month'] = $row->paymentamount;
        }
        foreach ($year_hall as $row) {
            $payment_hall[$row->paymenttype]['year'] = $row->paymentamount;
        }
		$data['payment_hall'] = $payment_hall;
...
// 		$this->load->view('reports/settlement_summary', $data);  
	...
	public function b2clarge($start_date = null, $to_date = null)
    {
        ...
        // $data['customerorder']  = $this->report_model->getcustomerorder($start_date, $to_date);
        $data['module'] = "reports";
        $data['page']   = "b2clarge";   
        echo Modules::run('template/layout', $data); 
...
public function hsnwise($start_date = null, $to_date = null)
    {
		...
		$to_date = $this->input->post('to_date', TRUE);
        // $data['csreport']  = $this->report_model->getcosummarylist($start_date, $to_date);
		$data['getfoodinfo']  = $this->report_model->getfoodinfo($start_date = NULL, $to_date = NULL);
		$data['gettariffinfo']  = $this->report_model->gettariffinfo($start_date = NULL, $to_date = NULL);
        $data['getbedinfo']  = $this->report_model->getbedinfo($start_date = NULL, $to_date = NULL);
...
public function b2csmall($start_date = null, $to_date = null)
    {
        ...
        $data['page']   = "b2csmall";   
        echo Modules::run('template/layout', $data); 
// 		$this->load->view('reports/b2csmall', $data);  
