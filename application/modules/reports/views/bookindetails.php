...
foreach ($booking_details as $key => $value) {
    ...
    if (!empty($value) && is_string($value)) {
        ...
        if ($key === 'bed_amount') {
            $bed_sum += $sum;
        } elseif ($key === 'person_amount') {
            $person_sum += $sum;
        } elseif ($key === 'child_amount') {
            $child_sum += $sum;
        }
    }
}
        ...
        ?>
            <div class="col-sm-6 text-right">
                ...
                <div class=" m-b-15"><?php echo display('payment_status') ?>:
                    ...
                    if($creditamt->credit>0){echo display("credit");}else
                    {echo "<span style='color:green;font-weight:bold;'>".display("paid")."</span>";
                    }}else if($bookinfo->paid_amount < $bookinfo->total_price*$datediff){ echo "<span style='color:red;font-weight:bold;'>". display("unpaid")."</span>";}else{ echo  "<span style='color:greenfont-weight:bold;'>". display("paid")."</span>";}?>
                ...
                <tbody>
                 ...
                 ?>
                    ...
                    <?php } ?>
                      <tr>
                        <td>
                            <div><strong><?php echo ('Room Rate') ?></strong></div>
                        </td>
                        <td><?php
                        $roomRatesArray = explode(",", $bookinfo->roomrate);
                        $totalRoomRate = 0;
                        foreach ($roomRatesArray as $rate) {$totalRoomRate += (int) $rate;}
                         echo  $totalRoomRate ; 
                           // echo html_escape($btaxinfo->remarks);
                        ?>
                        </td>
            ...
            <!-- <table class="table table-striped table-nowrap">
                <thead>
                    <tr>
                        ...
                        <th style="text-align: center;" ><?php echo display('price') ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                                    ...
                                    $x=0;
                                    $total=0;
                                    $roomId = explode(",", $bookinfo->room_no);
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;"><?php echo html_escape($alldays);?></td>
                      
                      
                      
                        <td style="text-align: center;"><?php echo html_escape($bookinfo->roomrate);?></td>
        ...
        <div class="row">
            ...
            <div class="col-sm-4">
                <ul class="list-unstyled text-right"  >
                    ...
                    <li>
                        <strong><?php echo display('discount'); ?>:</strong> <?php echo $disamount->discountamount;?>
                    </li>
                    <?php } ?>
                    <?php if($promocode>0){ ?>
                    <li>
                    ...
                    <li>
                        ...
                        <?php 
                        $ad_charge=($btaxinfo->additional_charges * 1.18 );

                     
                        echo html_escape($ad_charge);?>
                    </li>
                    <?php } ?>
                    ...
                    <li>
                        <?php $grprice += $scharge;?>
                         <strong><?php echo ('Advance Amount') ?>:</strong>
                        <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?><?php  
          
                        echo $bookinfo->advance_amount;?><?php if($currency->position==2){echo html_escape($currency->curr_icon);}?>
                        <br /><strong>
                        <strong><?php echo display('grand_total') ?>:</strong>
                        <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?><?php  
                       // echo $totalRoomRate."/".$bookinfo->cgst."/".$bookinfo->cgst."/".$ad_charge."/".($ad_charge*0.18)."/".$bed_sum."/".$person_sum."/".$child_sum."/".$disamount->discountamount;
                        // echo "<br/>";
                        echo $grand_total = number_format($totalRoomRate+$bookinfo->cgst+$bookinfo->cgst+$ad_charge+($ad_charge*0.18)+$bed_sum+$person_sum+$child_sum-$disamount->discountamount);?><?php if($currency->position==2){echo html_escape($currency->curr_icon);}?>
                        <br /><strong><?php echo display('paid_amount') ?>:</strong>
                        <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?><?php if (!empty($bookinfo->paid_amount)){$total_paid = $bookinfo->paid_amount+$bed_sum+$person_sum+$child_sum+$bookinfo->advance_amount+$promocode+$postedbill-$reducetax+ ($bookinfo->bookingstatus==5?$scharge:0);if($bookinfo->bookingstatus==5){echo $grand_total;}else{echo number_format($total_paid,2);}}?><?php if($currency->position==2){echo html_escape($currency->curr_icon);}?>
                        <br /><strong><?php echo display('due_amount') ?>:</strong>
                        <?php if($currency->position==1){echo html_escape($currency->curr_icon);}?>
                        <?php if (!empty($bookinfo->paid_amount)){
                        $remain_amount =($bookinfo->total_price+$bed_sum+$person_sum+$child_sum)-($bookinfo->paid_amount+$bookinfo->advance_amount);
                        if($remain_amount<0){echo "0";}else{echo $remain_amount;}} else echo html_escape($grprice+$totaltax+$scharge+$postedbill);?>
