...
<div class="card">
            ...
            <table id="exdatatable" class="table table-bordered table-striped table-hover">
                <thead>
                    ...
                    <th><?php echo ('Customer Name') ?></th>
                      <th><?php echo  ('Advance Id') ?></th>
                    <th><?php echo  ('Rate') ?></th>
                    <th><?php echo  ('Advance Paid Date') ?></th>
                    <th><?php echo  ('Advance Received') ?></th>
    ...
    <?php foreach ($getadvance as $report) : 
     $p=$report->advance_paydate;
    $p=explode(' ',$p);
    ?>
             <tr>
               <td><?php echo ($report->firstname." ".$report->lastname); ?></td>
                 <td><?php echo ($report->advance_id); ?></td>
               <td><?php echo html_escape('12'); ?></td>
                  <td><?php echo ($p[0]); ?></td>
                <td><?php echo html_escape($report->paymentamount); ?></td>
                 <td><?php echo html_escape($report->details); ?></td>
                <td><?php echo html_escape('0'); ?></td>
            </tr>
     <?php endforeach; ?>
      <?php foreach ($getadvanceinfo_hall as $report) : ?>
             <tr>
               <td><?php echo ($report->firstname." ".$report->lastname); ?></td>
                 <td><?php echo ($report->advance_id); ?></td>
               <td><?php echo html_escape('18'); ?></td>
                  <td><?php echo ($report->adv_date); ?></td>
                <td><?php echo html_escape($report->paymentamount); ?></td>
                 <td><?php echo html_escape($report->details); ?></td>
                <td><?php echo html_escape('0'); ?></td>
            </tr>
