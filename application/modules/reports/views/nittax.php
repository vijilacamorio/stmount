...
<div class="card">
 ...
 <?php $gst = ($report->total_amount - $report->discount) * (2.5 / 100);   ?>
 
<td><?php echo  $gst; ?></td>
<td><?php echo $gst; ?></td>
<td><?php echo $gst+$gst; ?></td>
<td><?php echo $report->discount ; ?></td>
<td><?php echo round(($report->total_amount) - ($report->discount))  ; ?></td>


 <td><?php echo html_escape(round($report->bill_amount)); ?></td>
 ...
 <td><?php echo html_escape($userinfo->firstname . ' ' . $userinfo->lastname); ?></td>
         ...
         $total_no_of_people += $sum_nuofpeople;
                ...
                $total_food_cgst += ($report->total_amount - $report->discount) * (2.5 / 100);
               
                $total_food_sgst += ($report->total_amount - $report->discount) * (2.5 / 100);
                $total_food_tax += (($report->total_amount - $report->discount) * (2.5 / 100))+(($report->total_amount - $report->discount) * (2.5 / 100));
                $discount += is_numeric($report->discount) ? $report->discount : 0;
                   $total_amount += html_escape($report->total_amount-$report->discount);
                $bill_amount += html_escape($report->bill_amount);
         $total_food_taxable_value = $bill_amount - $total_room_sgst - $total_room_cgst;
        } ?>
...
<tr>
        ...
        <td><?php echo  number_format(($total_food_cgst),3); ?></td>
        <td><?php echo  number_format(($total_food_sgst),3); ?></td>
        <td><?php echo  number_format(($total_food_tax),3); ?></td>
        <td><?php echo  $discount; ?></td>
        <td><?php echo round($total_amount); ?></td>
        <td><?php echo round($bill_amount); ?></td>
