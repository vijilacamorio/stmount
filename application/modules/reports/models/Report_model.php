...
$todate .= ' 23:59:59'; // Appending the time part
		$betweenq="booked_info.checkindate >='".$fromdate."' AND booked_info.checkoutdate <='".$todate."'";
		$this->db->select('booked_info.total_price as tp,tbl_postedbills.*,booked_info.*,booked_details.*,roomdetails.roomtype');
        $this->db->from('booked_info');
		$this->db->join('roomdetails','roomdetails.roomid=booked_info.roomid','left');
			$this->db->join('tbl_postedbills','tbl_postedbills.bookedid=booked_info.bookedid','left');
		$this->db->join('booked_details','booked_details.bookedid=booked_info.bookedid','left');
		if($fromdate != NULL){
...
public function food_info($start_date = NULL, $to_date = NULL) {
  ...
  if ($start_date && $to_date) {
        // Convert input dates to MySQL compatible format 'YYYY-MM-DD'
        $start_date_mysql = date('Y-m-d', strtotime($start_date));
        $to_date_mysql = date('Y-m-d', strtotime($to_date));

        // Build date conditions using STR_TO_DATE function for MySQL
        $this->db->where("STR_TO_DATE(b.bill_date, '%d-%m-%Y') BETWEEN '$start_date_mysql' AND '$to_date_mysql'");
    }
...
public function nightaudit()
{
 ...
 $query=$this->db->select('a.*, b.*')
         ...
         ->group_start() // Start grouping
             ->where('a.checkindate >=', $start_datetime)
             ->where('a.checkindate <=', $end_datetime)
         ->group_end() // End grouping
         ->group_start() // Start grouping for bookingstatus conditions
    ...
    foreach ($result as $row) {
        ...
        $processedData[] = [
         ...
         //   'adv'  => $row->advance_amount,
            ...
            'pc' => $row->pc,
            'cc' => $row->cc,
            'bed_amount' => array_sum(explode(',', $row->bed_amount)),
            'person_amount' => array_sum(explode(',', $row->person_amount)),
            'child_amount' => array_sum(explode(',', $row->child_amount)),
...
$start_datetime = date('Y-m-01', strtotime('01-' . $current_month)); 
$end_datetime = date('Y-m-t', strtotime('01-' . $current_month)); 
$query = $this->db->select('a.*, b.*')
                  ->from('booked_info a')
                  ->join('booked_details b', 'a.bookedid = b.bookedid')
                  ->group_start() 
                      ->where('STR_TO_DATE(a.checkindate, "%d-%m-%Y") >=', $start_datetime)
                      ->where('STR_TO_DATE(a.checkindate, "%d-%m-%Y") <=', $end_datetime)
                  ->group_end() 
                  ->group_start()
                      ->where('a.bookingstatus', 4)
                      ->or_where('a.bookingstatus', 5)
...
$start_datetime = date('Y-01-01', strtotime($current_year . '-01-01')); // Start of the year
$end_datetime = date('Y-12-31', strtotime($current_year . '-12-31')); // End of the year
$query = $this->db->select('a.*, b.*')
                  ->from('booked_info a')
                  ->join('booked_details b', 'a.bookedid = b.bookedid')
                  ->group_start()
...
public function getOccupancyData() {
    ...
    $this->db->where("bi.bookingstatus","4");
...
{
$this->db->select('tbl_postedbills.bc as bc,tbl_postedbills.pc as pc,tbl_postedbills.cc as cc,tbl_postedbills.additional_charges as addcharge,booked_info.*, roomdetails.*,  booked_details.*, 
...
$this->db->join(' tbl_postedbills ', 'booked_info.bookedid =  tbl_postedbills .bookedid', 'left');
...
$to_date1 .= ' 23:59:59'; // Appending the time part
 ...
 $this->db->group_by('booked_info.bookedid');
$query = $this->db->get();
//echo $this->db->last_query();die();
...
return false;
    ...
    $end_datetime = $today_date . ' 23:59:59';
		...
		$this->db->select('customerinfo.*,tbl_hallroom_booking.*,tbl_guestpayments.*,(tbl_guestpayments.paymentamount) ');
		$this->db->from('tbl_guestpayments');
			$this->db->join('tbl_hallroom_booking', ' tbl_hallroom_booking.invoice_no = tbl_guestpayments.invoice');
				$this->db->join('customerinfo', ' tbl_hallroom_booking.customerid = customerinfo.customerid');
...
$to_date1 .= ' 23:59:59'; // Appending the time part
    ...
    $end_datetime = $today_date . ' 23:59:59';
		...
		$this->db->select('booked_details.*,customerinfo.*,booked_info.*,tbl_guestpayments.*,(tbl_guestpayments.paymentamount)');
	   ...
	   $this->db->where('tbl_guestpayments.paymentamount = booked_details.advance_amount');
	   	$this->db->join('customerinfo', ' booked_info.cutomerid = customerinfo.customerid');
...
$to_date1 .= ' 23:59:59'; // Appending the time part
  ...
  //  echo $this->db->last_query(); die();
		...
		public function getsettlementsummaryinfo_room($start_date = NULL, $to_date = NULL)
	{
		...
		$this->db->select('tbl_postedbills.additional_charges as add_charges,tbl_guestpayments.paydate as paydate,booked_details.cgst as cgst,booked_details.sgst as sgst,tbl_guestpayments.*,booked_info.*,CONCAT_WS(" ", customerinfo.firstname,customerinfo.lastname) as customer_name');
		$this->db->from('booked_info');
		$this->db->join('customerinfo','customerinfo.customerid=booked_info.cutomerid','left');
		$this->db->join('booked_details','booked_details.bookedid=booked_info.bookedid','left');
		$this->db->join('tbl_postedbills','tbl_postedbills.bookedid=booked_info.bookedid','left');
...
public function read($limit = null, $start = null)
	{
	    $this->db->select('booked_info.total_price as tp,tbl_postedbills.*,booked_info.*,booked_details.*');
        $this->db->from('booked_info');
		$this->db->join('booked_details','booked_details.bookedid=booked_info.bookedid','left');
		$this->db->join('tbl_postedbills','tbl_postedbills.bookedid=booked_info.bookedid','left');
        $this->db->order_by('booked_info.bookedid', 'desc');
...
//     $to_date = date('Y-m-d', strtotime($to_date)) . ' 23:59:59';
    ...
    $this->db->join('tbl_hallroom_info hi', 'hb.hall_type = hi.hid', 'left');
    $this->db->join('customerinfo ci', 'ci.customerid = hb.customerid', 'left');
    $this->db->join('tbl_guestpayments gp', 'gp.bookedid = hb.hbid', 'left');
    if ($start_date) {
        $this->db->where("STR_TO_DATE(gp.paydate, '%d-%m-%Y') >=", $start_date);
        $this->db->where("STR_TO_DATE(gp.paydate, '%d-%m-%Y') <=", $to_date);
    }
    $this->db->where("gp.book_type",'1');
     $this->db->group_by('gp.bookedid');
    $query = $this->db->get();
  //  echo $this->db->last_query();die();
    if ($query === FALSE) {
        $error = $this->db->error();
...
public function xmlroomdata_report($start_date=null,$to_date=null){
     $start_date = $start_date . ' 00:00:00';
    $to_date = $to_date . ' 23:59:59';
    $this->db->select('booked_info.*, booked_details.*, tbl_guestpayments.*, customerinfo.*');
    $this->db->from('booked_info');
    $this->db->join('booked_details', 'booked_details.bookedid = booked_info.bookedid');
    $this->db->join('tbl_guestpayments', 'tbl_guestpayments.bookedid = booked_info.bookedid');
    
    $this->db->join('customerinfo', 'customerinfo.customerid = booked_info.cutomerid');
    $this->db->where('tbl_guestpayments.book_type', 0);
     if ($start_date) {
        $this->db->where("tbl_guestpayments.paydate >=", $start_date);
        $this->db->where("tbl_guestpayments.paydate <=", $to_date);
...
// Xml Hall report Data
public function xmlhalldata_report($start_date=null,$to_date=null){
     $start_date = $start_date . ' 00:00:00';
    $to_date = $to_date . ' 23:59:59';
    $this->db->select('tbl_hallroom_info.*, tbl_hallroom_booking.*, tbl_guestpayments.*, customerinfo.*');
    $this->db->from('tbl_hallroom_info');
    $this->db->join('tbl_hallroom_booking', 'tbl_hallroom_booking.hall_type = tbl_hallroom_info.hid');
    $this->db->join('tbl_guestpayments', 'tbl_guestpayments.bookedid = tbl_hallroom_booking.hbid');
    $this->db->join('customerinfo', 'customerinfo.customerid = tbl_hallroom_booking.customerid');
    $this->db->where('tbl_guestpayments.book_type', 1);
     if ($start_date) {
        $this->db->where("tbl_guestpayments.paydate >=", $start_date);
        $this->db->where("tbl_guestpayments.paydate <=", $to_date);
...
// Xml Restaurtant report Data
public function xmlrestaurtantdata_report($start_date=null,$to_date=null){
   
    $this->db->select('customer_order.*, bill.*, customerinfo.*');
    $this->db->from('customer_order');
    $this->db->join('bill', 'bill.order_id = customer_order.order_id');
    $this->db->join('customerinfo', 'customerinfo.customerid = customer_order.customer_id');
     if ($start_date) {
        $this->db->where("bill.bill_date >=", $start_date);
        $this->db->where("bill.bill_date <=", $to_date);
    }
    $query = $this->db->get();
