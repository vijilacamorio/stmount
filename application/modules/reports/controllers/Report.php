...
// 		$this->load->view('reports/advancereceived', $data);  
       ...
       public function generatexmldata()
    {
		...
		$to_date = $this->input->post('end', TRUE);
		
		$this->permission->method('reports','read')->redirect();
		$data['title']    = ('Transaction Report'); 
        $data['roomdatareport']  = $this->report_model->xmlroomdata_report($start_date,$to_date);
        $data['halldatareport']  = $this->report_model->xmlhalldata_report($start_date,$to_date);
        $data['restaurtantdatareport']  = $this->report_model->xmlrestaurtantdata_report($start_date,$to_date);
      
        // $data['restaurtantpayment']  = $this->report_model->getrestaurant_payment();
      //  print_r($data['restaurtantdatareport']);die();
        $data['module'] = "reports";
        $data['page']   = "generate_xml";    
		$this->load->view('reports/generate_xml', $data);  
    }
