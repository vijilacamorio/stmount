...
class Item_food extends MX_Controller {
    ...
    public function index()
    {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('ordermanage/item_food/index');
    ...
    public function create($id = null)
    {
	  ...
	  /****************image Upload*************/
	    $config['upload_path']          = 'application/modules/ordermanage/assets/images/';
		...
		{		
            ...
            foreach ($image_sizes as $key=>$resize) {
             ...
             $this->image_lib->resize();  
	    $big='application/modules/ordermanage/assets/images/big/'.$fdata['file_name'];
		$medium='application/modules/ordermanage/assets/images/medium/'.$fdata['file_name'];
		$small='application/modules/ordermanage/assets/images/small/'.$fdata['file_name'];
		$img = 'application/modules/ordermanage/assets/images/'.$fdata['file_name'];          
		}
	  ...
	  /****************end*********************/
	   ...
	   'DateInserted'       	=> date('d-m-Y H:i:s'),
	   'DateUpdated'        	=> date('d-m-Y H:i:s'),
	   'DateLocked'         	=> date('d-m-Y H:i:s'),
	  );
	  $logData = array(
	  ...
	  $taxsettings = $this->taxchecking();
		...
		$this->logs_model->log_recorded($logData);
		 ...
		 redirect('ordermanage/item_food/create');
		} else {
		...
		$this->permission->method('ordermanage','update')->redirect();
		if(empty($img)){
	  ...
	  $taxsettings = $this->taxchecking();
		...
		redirect("ordermanage/item_food/create/".$postData['ProductsID']);  
	   }
	  } else { 
    ...
    {
        $this->permission->module('ordermanage','delete')->redirect();
		$logData = [
	   'action_page'         => "Food List",
	   ...
	   'entry_date'          => date('d-m-Y H:i:s'),
		...
		redirect('ordermanage/item_food/index');
    }
		...
		$data['intinfo']="";
			if ($this->form_validation->run()) { 
				...
				if ($this->fooditem_model->addsupplier($postData)) { 
					$this->logs_model->log_recorded($logData);
					$this->session->set_flashdata('message', display('save_successfully'));
					redirect('ordermanage/item_food/bulkfoodupload');
				} else {
					$this->session->set_flashdata('exception',  display('please_try_again'));
				}
				redirect("ordermanage/item_food/bulkfoodupload"); 
	...
	public function foodvarientlist($id=null)
    {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('ordermanage/item_food/foodvarientlist');
	 ...
	 $logData = [
	   ...
	   'entry_date'          => date('d-m-Y H:i:s'),
		...
		if ($this->foodvarient_model->create($postData)) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage/item_food/foodvarientlist');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage/item_food/foodvarientlist"); 
   ...
   public function updateintfrm($id){
	   ...
	   $data['currency']=$this->fooditem_model->currencysetting($settinginfo->currency);
		...
		$this->load->view('ordermanage/varientedit', $data);   
	...
	public function availablelist($id=null)
    {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('ordermanage/item_food/availablelist');
	 ...
	 $logData = [
	   ...
	   'entry_date'          => date('d-m-Y H:i:s'),
		...
		if ($this->foodavailable_model->create($postData)) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage/item_food/availablelist');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage/item_food/availablelist"); 
   ...
   public function updateavailfrm($id){
		...
		$this->load->view('ordermanage/availabledit', $data);   
	...
	public function menutypecreate($id = null)
    {
	  ...
	  $img = $this->fileupload->do_upload('./application/modules/ordermanage/assets/images/','picture');
	 ...
	 $logData = array(
	   ...
	   'entry_date'          => date('d-m-Y H:i:s'),
		...
		if ($this->todaymenu_model->create($postData)) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage/item_food/todaymenutype');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage/item_food/todaymenutype"); 
   ...
   public function updatemenufrm($id){
        ...
        $data['module'] = "ordermanage";  
        $data['page']   = "mtypeedit";
		$this->load->view('ordermanage/mtypeedit', $data);   
	   }
	...
	public function addgroupfood($id = null)
    {
	  ...
	  if($totalitem<2){
		   ...
		   redirect("ordermanage/item_food/addgroupfood");
		   }
	  /****************image Upload*************/
	    $config['upload_path']          = 'application/modules/ordermanage/assets/images/';
		$config['allowed_types']        = 'gif|jpg|jpeg|png';
	  ...
	  $taxsettings = $this->taxchecking();
		...
		redirect("ordermanage/item_food/addgroupfood/".$postData['ProductsID']);  
	   }
	  } else { 
	  $data['taxitems'] = $this->taxchecking();
	   if(!empty($id)) {
