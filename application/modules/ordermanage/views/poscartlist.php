...
<table class="table table-bordered" border="1" width="100%" id="addinvoice">
    ...
    <tbody class="itemNumber">
        ...
        <tr id="<?php echo $i;?>">
            ...
            <?php if ($item['size'] == '77'): ?>
              <td width="">
              <input type="text" style="width: 60px; border:none;text-align: center;" class="new_price" id="new_price_<?php echo $item['rowid']; ?>" name="new_price" value="<?php echo $item['price']; ?>" onblur="updateCartItem(this.value, '<?php echo $item["rowid"]; ?>')">
        <input type="hidden" class="get_varientid" name="variantName" value="<?php echo $item['size']; ?>">
      </td>
<?php else: ?>
...
<?php endif; ?>
           ...
           <?php if ($item['size'] == '77'): ?>
    <td width="">
        <input type="text" style="width: 60px; border:none;text-align: center;" id="total_new_price_<?php echo $item['rowid']; ?>" name="total_new_price" value="<?php echo $itemprice-$mypdiscount;?>"  >
     </td>
<?php else: ?>
    <td>   <?php echo $itemprice-$mypdiscount;?>  </td>
<?php endif; ?>
...
<script>
    function updateCartItem(obj, rowid){
            ...
            var csrfToken = $('#csrfhashresarvation').val();
            $.get("<?php echo base_url("ordermanage/order/updated_food_price") ?>", {csrf_test_name: csrfToken, rowid: rowid, price: price}, function(resp){
                if(resp === 'ok'){
                    location.reload();
                }else{
                    $('#new_price_' + rowid).val(price);
                    $('#total_new_price_' + rowid).val(price);
                }
            });
        }
