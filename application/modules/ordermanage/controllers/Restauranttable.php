...
class Restauranttable extends MX_Controller {
    ...
    public function index($id = null)
    {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('ordermanage/restauranttable/index');
	 ...
	 $logData = [
	  ...
	  $lastid=$this->restable_model->create($postData);
		...
		'tableid'         => $lastid,
		 ...
		 redirect('ordermanage/table-list');
		} else {
   ...
   public function updateintfrm($id){
        ...
        $data['module'] = "ordermanage";  
        $data['page']   = "tableedit";
		$this->load->view('ordermanage/tableedit', $data);   
	   }
	...
	public function showfile(){
			    $data['module'] = "ordermanage";  
				$data['page']   = "alllayout";
				$this->load->view('ordermanage/alllayout', $data);
	 ...
	 $logData = array(
	  ...
	  $lastid=$this->restable_model->createfloor($postData);
		if($lastid) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage/restauranttable/floorlist');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage/restauranttable/floorlist"); 
	 ...
	 $logData = array(
	  ...
	  $lastid=$this->restable_model->createroom($postData);
		if($lastid) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('ordermanage/restauranttable/roomlist');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("ordermanage/restauranttable/roomlist"); 
...
public function updateroomfrm($id){
        ...
        $data['module'] = "ordermanage";  
        $data['page']   = "roomedit";
		$this->load->view('ordermanage/roomedit', $data);   
	   }
