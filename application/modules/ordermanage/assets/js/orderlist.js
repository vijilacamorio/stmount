...
$(document).ready(function () {
"use strict";
     var allordertable = $('#tallorder').DataTable({ 
        responsive: true, 
        paging: true,
		"language": {
		...
		"searching": true,
		  "processing": true,
				 "serverSide": true,
				 "ajax":{
					url :baseurl+"ordermanage/order/allorderlist",
					type: "post",
					"data": function ( data ) {
            ...
            allordertable.on( 'draw', function () {
...
function noteCheck(id){
	 ...
	 var myurl =baseurl+'ordermanage/order/ajaxupdateoreder/';
...
function createMargeorder(orderid,value=null){
	...
	$("#account_number").val('');
		...
		$(element).closest('div.row').next().find('div.cardarea').removeClass("display-none");
		$(element).closest('div.row').next().find('div.cardarea2').addClass("display-none");
		$(element).closest('div.row').next().find('div.cardarea6').removeClass("display-none");
		$("#assigncard_terminal").val('');
		$("#assignbank").val('');
		$("#assignlastdigit").val('');
		$("#account_number").val('');
	  ...
	  function showhide(id){
		   ...
		   var isVisible = $( '#item'+id ).is(".active");
			if (isVisible === true ){
				...
				$.ajax({
				type: "POST",
				url: baseurl+"ordermanage/order/itemlist",
				data: dataString,
				success: function(data){
					$('#item'+id).html(data);
