...
class Order_model extends CI_Model {
	...
	public function allfood(){
		...
		public function multibill($id = null){
            $this->db->select('*');
	...
	public function bankheadcode($lvl,$code){
...
// 		echo $this->db->last_query(); die();
	...
	//	echo $this->db->last_query();
		...
		$customerinfo=$this->read('*', 'customerinfo', array('customerid' => $cid));
...
//	echo $this->db->last_query();die();
	...
	public function billinfo($id = null){
		...
		$this->db->where('bill.order_id',$id);
		$query = $this->db->get();
	...
	{ 
		...
		$this->db->join('variant','item_foods.ProductsID=variant.menuid','left');
		$this->db->where('variant.menuid',$id);
		$this->db->where('variant.variantid',$sid);
		$query = $this->db->get();
		$itemlist=$query->row();
	    return $itemlist;
...
public function customer_dropdown2()
{
    // Get the timezone from the settings table
    $timezone = $this->db->select('timezone')->from('setting')->get()->row();
    date_default_timezone_set($timezone->timezone);

    // Get the current date and time in the format 'd-m-Y H:i:s'
    $cr_time = date("d-m-Y H:i:s");
    $today = date("d-m-Y");
    ...
    $sql = "SELECT * 
            ...
            WHERE 
               booked_info.checkindate <= '$cr_time' AND 
               booked_info.checkoutdate >= '$today' AND 
                `booked_info`.`bookingstatus` = 4";

    $query = $this->db->query($sql); // Execute the query
    //echo $this->db->last_query();die(); // Uncomment to see the generated SQL query
    ...
    if (!empty($data)) {
        foreach ($data as $value) {
            $uniqueKey = $value->customerid . '_' . $value->booking_number;
            $list[$uniqueKey] = $value->firstname . ' ' . $value->lastname . ' (' . $value->room_no . ')';
        }
    }
...
public function customer_dropdown3()
{
    // Get the timezone from the settings table
    $timezone = $this->db->select('timezone')->from('setting')->get()->row();
    ...
    $query = $this->db->query($sql); // Execute the query
    // Uncomment to see the generated SQL query
    // echo $this->db->last_query(); die();
    ...
    if (!empty($data)) {
        foreach ($data as $value) {
            // Corrected the concatenation error here
            $uniqueKey = $value->customerid . '_' . $value->firstname;
            $list[$uniqueKey] = $value->title . ' ' . $value->firstname . ' ' . $value->lastname . ' (' . $value->firstname . ' ' . $value->lastname . ')';
        }
    }
	...
	public function oldcustomer_dropdown()
	{
		...
		if (!empty($data)) {
			foreach($data as $value)
				$list[$value->customerid] = $value->cust_phone;
			return $list;
		} else {
			return false; 
		}
 ...
 public function ctype_dropdown()
  ...
  public function bank_dropdown()
	{
		$query=$this->db->query("SELECT * FROM acc_coa WHERE HeadLevel='4' And HeadCode LIKE '1020102%'");
		$allbankdata = $query->result();

		$list[''] = 'Select Bank';
		if (!empty($allbankdata)) {
			foreach($allbankdata as $value)
 ...
 public function waiter_dropdown()
	{
 	...
 	public function rosterwisecustomer()
	{
		$timezone = $this->db->select('timezone')->from('setting')->get()->row();
        date_default_timezone_set($timezone->timezone);
 ...
 public function pmethod_dropdown(){
  ...
  public function read($select_items, $table, $where_array)
	...
	public function orderlist($limit = null, $start = null){
			$this->db->select('customer_order.*,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
			$this->db->from('customer_order');
    ...
    public function pendingorder($limit = null, $start = null, $status= null){
		$sql="SELECT customer_order.*,CONCAT_WS(' ', customerinfo.firstname, customerinfo.lastname) AS customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename 
		FROM customer_order 
		LEFT JOIN customerinfo ON customerinfo.customerid=customer_order.customer_id 
		LEFT JOIN customer_type ON customer_type.customer_type_id=customer_order.cutomertype 
		LEFT JOIN employee_history ON employee_history.emp_his_id=customer_order.waiter_id 
		LEFT JOIN rest_table ON rest_table.tableid=customer_order.table_no 
		WHERE customer_order.order_status = $status 
		ORDER BY customer_order.order_id DESC";
	...
	public function completeorder($limit = null, $start = null, $status= null){
		$sql="SELECT customer_order.*,CONCAT_WS(' ', customerinfo.firstname, customerinfo.lastname) AS customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename 
		FROM customer_order 
		LEFT JOIN customerinfo ON customer_order.customer_id=customerinfo.customerid 
	...
	public function kitchen_ajaxorderinfo($id){
		$this->db->select('customer_order.*,customerinfo.firstname,customerinfo.memberid,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
        $this->db->from('customer_order');
		$this->db->join('customerinfo','customer_order.customer_id=customerinfo.memberid','left');
		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
		$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
		$this->db->join('rest_table','customer_order.table_no=rest_table.tableid','left');
	...
	public function uniqe_order_id($id){
		...
		return false;
			...
			public function merge_amount($id){
		$this->db->select('*');
        $this->db->from('customer_order');
    	$this->db->join('bill','customer_order.order_id=bill.order_id','left');
		$this->db->where('customer_order.order_id',$id);
   ...
   public function new_entry($data = array())
	...
	public function get_ongoingorder(){
		...
		AND ((customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) 
		|| (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) 
		|| (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) 
		|| (customer_order.cutomertype = 6 || customer_order.orderacceptreject != 1) 
		|| (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";

		$sql="SELECT customer_order.*,customer_order.order_id as mid,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename 
		FROM customer_order 
		Left JOIN customerinfo ON customer_order.customer_id=customerinfo.customerid 
		...
		UNION SELECT customer_order.*,customer_order.order_id as mid,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename 
		FROM customer_order 
		Left JOIN customerinfo ON customer_order.customer_id=customerinfo.customerid 
		Left JOIN booked_info ON customer_order.customer_id=booked_info.cutomerid 
		Left Join customer_type ON customer_order.cutomertype=customer_type.customer_type_id 
		left join employee_history ON customer_order.waiter_id=employee_history.emp_his_id 
		Left Join rest_table ON customer_order.table_no=rest_table.tableid 
		Where {$where} AND customer_order.marge_order_id IS NOT NULL 
		GROUP BY customer_order.marge_order_id order by mid desc";
		...
		Left Join customer_type ON customer_order.cutomertype=customer_type.customer_type_id 
		left join employee_history ON customer_order.waiter_id=employee_history.emp_his_id 
		Left Join rest_table ON customer_order.table_no=rest_table.tableid Where {$where} 
		AND customer_order.marge_order_id IS NOT NULL 
		...
		$sql="SELECT customer_order.*,customer_order.order_id as mid,customerinfo.firstname,customer_type.customer_type,
		employee_history.first_name,employee_history.last_name,rest_table.tablename 
		FROM customer_order 
		Left JOIN customerinfo ON customer_order.customer_id=customerinfo.customerid 
		Left Join customer_type ON customer_order.cutomertype=customer_type.customer_type_id 
		left join employee_history ON customer_order.waiter_id=employee_history.emp_his_id 
		Left Join rest_table ON customer_order.table_no=rest_table.tableid 
		Where {$where} AND customer_order.marge_order_id 
		IS NULL UNION SELECT customer_order.*,customer_order.order_id as mid,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename 
		FROM customer_order Left JOIN customerinfo ON customer_order.customer_id=customerinfo.customerid 
		Left Join customer_type ON customer_order.cutomertype=customer_type.customer_type_id 
		left join employee_history ON customer_order.waiter_id=employee_history.emp_his_id 
		Left Join rest_table ON customer_order.table_no=rest_table.tableid 
		Where {$where} AND customer_order.marge_order_id IS NOT NULL GROUP BY customer_order.marge_order_id 
		order by mid desc";
		$query=$this->db->query($sql);
	...
	{
			$column_order = array(null, 'customer_order.saleinvoice','customerinfo.customerid','customer_type.customer_type','CONCAT_WS(" ", employee_history.first_name,employee_history.last_name)','rest_table.tablename','customer_order.order_date','customer_order.totalamount'); //set column field database for datatable orderable
			$column_search = array('customer_order.saleinvoice','customerinfo.customerid','customer_type.customer_type','CONCAT_WS(" ", employee_history.first_name,employee_history.last_name)','rest_table.tablename','customer_order.order_date','customer_order.totalamount'); //set column field database for datatable searchable 
			$order = array('customer_order.order_id' => 'asc');
			$this->db->select('customer_order.*,CONCAT_WS(" ",customerinfo.firstname,customerinfo.lastname) as customer_name,customerinfo.customerid,customer_type.customer_type,CONCAT_WS(" ", employee_history.first_name,employee_history.last_name) AS fullname,rest_table.tablename');
			$this->db->from('customer_order');
			$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
			$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
			$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
	...
	{
		    $this->db->select('customer_order.*,customerinfo.firstname,customerinfo.customerid,customer_type.customer_type,CONCAT_WS(" ", employee_history.first_name,employee_history.last_name) AS fullname,rest_table.tablename');
			$this->db->from('customer_order');
			$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
			$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
			$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
	...
	public function kitchen_ongoingorder($id){
		...
		$this->db->select('customer_order.*,item_foods.kitchenid,order_menu.menu_id,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
        $this->db->from('customer_order');
		$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
		$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
	...
	{
		$column_order = array(null, 'customer_order.saleinvoice','customerinfo.firstname','customer_order.food_need_date','item_category.Name','item_foods.ProductName','order_menu.menuqty','order_menu.price'); //set column field database for datatable orderable
			$column_search = array('customer_order.saleinvoice','customerinfo.firstname','customer_order.food_need_date','item_category.Name','item_foods.ProductName','order_menu.menuqty','order_menu.price'); //set column field database for datatable searchable 
			$order = array('customer_order.order_id' => 'asc');
		
		$cdate=date('d-m-Y');
		$this->db->select('order_menu.price as price ,order_menu.menuqty as  qty, item_foods.*,order_menu.*,customer_order.*,CONCAT_WS(" ", customerinfo.firstname,customerinfo.lastname) as customer_name,bill.bill_status,customer_order.food_need_date as food_date,item_foods.ProductName as item,customer_order.totalamount as totalamount,item_category.Name as category,order_menu.order_id as order_id');
         $this->db->from('customer_order');
        	$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
	...
	{
		$cdate=date('d-m-Y');
		$this->db->select('customer_order.*,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename,bill.bill_status');
        $this->db->from('customer_order');
		$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
		$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
	...
	{
			$column_order = array(null, 'customer_order.saleinvoice','customerinfo.firstname','customer_type.customer_type','employee_history.first_name','employee_history.last_name','rest_table.tablename','customer_order.order_date','customer_order.totalamount'); //set column field database for datatable orderable
			$column_search = array('customer_order.saleinvoice','customerinfo.firstname','customer_type.customer_type','employee_history.first_name','employee_history.last_name','rest_table.tablename','customer_order.order_date','customer_order.totalamount'); //set column field database for datatable searchable 
			$order = array('customer_order.order_id' => 'asc');
		
		$cdate=date('d-m-Y');
		$previousday = date('d-m-Y', strtotime($cdate. ' -2 days'));
		$condi = "customer_order.order_date BETWEEN '".$previousday."' AND '".$cdate."'";

		$this->db->select('customer_order.*, CONCAT_WS(" ", customerinfo.firstname, customerinfo.lastname) AS customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename,bill.bill_status,bill.shipping_type');
        $this->db->from('customer_order');
		$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
		$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
	...
	public function count_allonlineorder()
	{
		...
		$previousday = date('d-m-Y', strtotime($cdate. ' -2 days'));
		$condi = "customer_order.order_date BETWEEN '".$previousday."' AND '".$cdate."'";
		$this->db->select('customer_order.*,customerinfo.firstname,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename,bill.bill_status,bill.shipping_type');
        $this->db->from('customer_order');
		$this->db->join('customerinfo','customer_order.customer_id=customerinfo.customerid','left');
		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
		$this->db->join('employee_history','customer_order.waiter_id=employee_history.emp_his_id','left');
...
public function get_orderlist(){
		...
		AND ((customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3) 
		AND ((customer_order.cutomertype = 2 AND customer_order.orderacceptreject = 1) 
		|| (customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) 
		|| (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) 
		|| (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) 
		|| (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";
	...
	public function get_table_total_customer($id){
		$where = "table_id = '".$id."' AND delete_at = 0 AND created_at= '".date('d-m-Y')."'";
    ...
    public function get_table_total($floorid){
    	$where = "table_details.delete_at = 0 AND table_details.created_at= '".date('d-m-Y')."'";
	...
	public function showsplitorderlist($order){
		$this->db->select('sub_order.*,CONCAT_WS(" ",customerinfo.firstname,customerinfo.lastname) as customer_name');
        $this->db->from('sub_order');
		$this->db->join('customerinfo','sub_order.customer_id=customerinfo.customerid','left');
