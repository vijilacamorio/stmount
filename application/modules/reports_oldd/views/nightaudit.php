...
echo '<tr><td style="font-weight:bold;">Tariff</td><td>' . ($obj['total_price'] && !empty($checkuser) ? ($obj['total_price']) : '0.00') . '</td><td>' . ($month['total_price'] ? ($month['total_price']) : '0.00') . '</td><td>' . ($year['total_price'] ? $year['total_price'] : '0.00') . '</td></tr>';
echo '<tr><td style="font-weight:bold;">Room Rent</td><td>' . ($obj['roomrate'] && !empty($checkuser) ? $obj['roomrate']-$obj['discountamount'] : '0.00') . '</td><td>' . ($month['roomrate'] ? $month['roomrate']-$month['discountamount'] : '0.00') . '</td><td>' . ($year['roomrate'] ? $year['roomrate']-$year['discountamount'] : '0.00') . '</td></tr>';
 
echo '<tr><td style="font-weight:bold;">Extra Bed</td><td>' . ($obj['bc'] && !empty($checkuser) ? $obj['bc'] : '0.00') . '</td><td>' . ($month['bc'] ? $month['bc'] : '0.00') . '</td><td>' . ($year['bc'] ? $year['bc'] : '0.00') . '</td></tr>';
echo '<tr><td style="font-weight:bold;">Extra Person</td><td>' . ($obj['pc'] && !empty($checkuser) ? $obj['pc'] : '0.00') . '</td><td>' . ($month['pc'] ? $month['pc'] : '0.00') . '</td><td>' . ($year['pc'] ? $year['pc'] : '0.00') . '</td></tr>';
echo '<tr><td style="font-weight:bold;">Extra Child</td><td>' . ($obj['cc'] && !empty($checkuser) ? $obj['cc'] : '0.00') . '</td><td>' . ($month['cc'] ? $month['cc'] : '0.00') . '</td><td>' . ($year['cc'] ? $year['cc'] : '0.00') . '</td></tr>';
...
<td colspan="4" style="font-size:15px;text-align:center;font-weight:bold;">Miscelleneous</td>
                                    ...
                                    <tbody>';
echo '<tr><td style="font-weight:bold;">Additional Charge</td><td>' . ($obj_post->additional_charges && !empty($checkuser) ? $obj_post->additional_charges : '0.00') . '</td><td>' . ($month_post->additional_charges ? $month_post->additional_charges : '0.00') . '</td><td>' . ($year_post->additional_charges ? $year_post->additional_charges : '0.00') . '</td></tr>';
echo "</tbody></table>";
...
<td colspan="4" style="font-size:15px;text-align:center;font-weight:bold;">Room Revenue</td>
                                    ...
                                    <tbody>';
 echo '<tr><td style="font-weight:bold;">Amount Received</td><td>' . (!empty($nightaudit_advance[0]->paid_amount  && !empty($checkuser) ) ? ($nightaudit_advance[0]->paid_amount): '0.00') . '</td><td>' . ($nightaudit_advance_m[0]->paid_amount ? ($nightaudit_advance_m[0]->paid_amount) : '0.00') . '</td><td>' . ($nightaudit_advance_y[0]->paid_amount ? ($nightaudit_advance_y[0]->paid_amount ): '0.00') . '</td></tr>';
 echo '<tr><td style="font-weight:bold;">Due Amount</td><td>' . (!empty($obj['total_price']) ?($obj['total_price']-$obj['paid_amount']) : '0.00') 
 . '</td><td>' . ($month['total_price'] ? ($month['total_price']-$month['paid_amount']) : '0.00') . '</td><td>' . ($year['total_price'] ? ($year['total_price']-$year['paid_amount']) : '0.00') . '</td></tr>';
echo "</tbody></table>";     
...
<?php endif; ?>
  ...
  <table border="1" class="table table-bordered table-striped table-hover">
    ...
    <?= !empty($checkuser) ? (isset($data['today']) ? number_format($data['today'], 2) : '0.00') : '' ?>
                ...
                <td><?= number_format($data['month'], 2) ?></td>
                <td><?= number_format($data['year'], 2) ?></td>
            </tr>
        <?php endforeach; ?>
        <tr>
            <td>Total</td>
     ...
     <tbody>
        ...
        foreach ($paymentTypes as $type) {
         ...
         echo "<td>" . (!empty($checkuser) && isset($sumToday[$type]) ? $sumToday[$type] : 0) . "</td>";
            ...
            echo "<td>" . ($sumMonth[$type] ?? 0) . "</td>";
            echo "<td>" . ($sumYear[$type] ?? 0) . "</td>";
            echo "</tr>";

            // Accumulate totals for each column
            $totalToday1 += $sumToday[$type] ?? 0;
            $totalMonth1 += $sumMonth[$type] ?? 0;
            $totalYear1 += $sumYear[$type] ?? 0;
        }
        ?>
    </tbody>
    <tfoot>
