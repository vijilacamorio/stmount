...
$("#amount").on("keyup change", function() {
      debugger;
    var amount = customRound(parseFloat($("#amount").val()));
    var additionalcharge = parseFloat($("#additionalcharge").val());
    ...
    var final = customRound(totalrentandtax + CGST + SGST);
    var payableamount = customRound(final - alladvanceamount   +additionalcharge) ;
    var payableamt = customRound(payableamount + poolbill + restbill + hallbill + parkingbill) ;
     var rentandextras=(allbpccharge+roomrent);
    if (amount >= 0 && amount <= roomrent) {
        $("#percent").val((amount * 100) / roomrent);
        $("#disamount").text(amount);
        $("#totalroomrentamount").text(final);
        $('#rentafterdiscount').val(final);
        $("#allroomrentandtax").text(final);
         $("#sub").text('₹'+totalrentandtax.toFixed(2));
        $("#payableamount").text(customRound(final - alladvanceamount));
        $("#netpayableamount").text(payableamount);
        $("#payableamt").text(payableamount);
        $("#balance").text(payableamount);
...
var payableamt = customRound($("#payableamt").text());
$("#balance").text($("#payableamt").text());
"use strict";
   ...
   if (isNaN(additionalcharge)) {
    ...
    if (isNaN(complemetaryamt)) {
      ...
      if (isNaN(amount)) {
        ...
        var CGST = (totalrentandtax * 6) / 100;
        var SGST = (totalrentandtax * 6) / 100;
        var final =customRound(totalrentandtax +
          CGST +
          SGST +
          additionalcharge -
          alladvanceamount);
        var payableamount =
          customRound(totalrentandtax -
          alladvanceamount -
          amount +
          additionalcharge);
        var payableamt =
          customRound(payableamount +
          poolbill +
          restbill +
          hallbill +
          parkingbill +
          additionalcharge);
        var rentandextras = customRound(allbpccharge + roomrent);
        ...
        $("#netpayableamount").text(final);
        $("#payableamt").text(final);
        $("#balance").text(final);
...
$("#creditamount").on("keyup change", function() {
    ...
    if (creditamount >= 0) {
        ...
        $("#balance").text((payableamt - creditamount + additionalcharge - complemetaryamt).toFixed(2));
...
$("#credit").on("change", function() {
    ...
    if (credit) {
        ...
        $("#balance").text((payableamt + creditamount).toFixed(2));
        $("#creditamount").val(0);
        $("#creditamt").text(0);
...
$("#complementary").on("change", function() {
    ...
    if (complementary) {
        ...
        $("#balance").text((payableamt + complementaryamount).toFixed(2));
 ...
 $("#bankname_" + l).prop("style", "display:none;");
  ...
  $("#bankname_" + l).prop("style", "display:none;");
    ...
    $("#cash_" + l + "").on("keyup change", function() {
        ...
        var balance = parseFloat($("#payableamt").text().replace(/,/g, ''));
        var invcredit = $("#creditamount").val();
