...
?>
    ...
    <body>
        ...
        <div class="invoice-wrap print-pool invp-1">
                ...
                <div class="invp-11">
                    ...
                    <div>
                        <address class="invp-12">
                            ...
                            <?php if($poolcastinfo->firstname){?><span class="invp-10"><?php echo display("guests_name") ?>
                                :&nbsp;</span><?php echo html_escape($poolcastinfo->firstname); ?><br><?php }?>
                            <?php if($poolcastinfo->cust_phone){?><span
                                class="invp-10"><?php echo display("mobile") ?>:&nbsp;</span><?php if($poolcastinfo->cust_phone){echo html_escape($poolcastinfo->cust_phone);} ?>
                            <br><?php }?>
                            <?php if($poolcastinfo->email){?><span
                                class="invp-10"><?php echo display("email") ?>:&nbsp;</span><?php if($poolcastinfo->email){echo html_escape($poolcastinfo->email);}?><?php }?>
                        </address>
                    </div>
                </div>
            </div>
            ...
            <table
                ...
                <tbody>
                     <?php for($i=0;$i<count($pitemlist); $i++){ ?>   
                        <?php for($j=0;$j<count($pitemlist[$i]); $j++){ ?>   
                            <?php $sl = 1;?>
                            <?php foreach ($pitemlist[$i][$j] as $row) { ?>
                            <tr>
                                <td class="invp-14"><?php echo $sl; ?></td>
                                <td class="invp-14">
                                    <?php echo html_escape($row->package_name); ?></td>
                                <td class="invp-14"> <?php echo html_escape($row->perprice); ?>
                                </td>
                                <td class="invp-14"> <?php echo html_escape($row->itemqty); ?>
                                </td>
                                <td class="invp-14">
            ...
            <table border="0" cellpadding="0" cellspacing="0" align="center" class="invp-24">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="invp-25">
                            <strong><?php if($currency->position==1){ echo html_escape($currency->curr_icon); } ?> 
                            <?php $paidtotal = 0; ?>   
                            <?php for($i=0;$i<count($pitemdata); $i++){ ?>   
                                <?php for($j=0;$j<count($pitemdata[$i]); $j++){ ?> 
                                    <?php $paidtotal+=$pitemdata[$i][$j]->total_amount; ?>
                                <?php } ?>
                            <?php } ?> <?php echo html_escape($paidtotal); ?>
    ...
    <script src="<?php echo MOD_URL;?>room_reservation/assets/js/poolprint.js"></script>
