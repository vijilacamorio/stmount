        ...
        <div class="card mb-4 flex-fill w-100">
            ...
            <div class="row p-3" id="allRooms">
                ...
                <?php 
                    ...
                    ?>
                <?php 
                $id = $this->db->select("bookedid,bookingstatus")->from("booked_info")->where("checkindate<=",date("d-m-Y H:i:s"))->where("checkoutdate>=",date("d-m-Y H:i:s"))->where("FIND_IN_SET(".$list->roomno.",room_no)<>",0)->where_in("bookingstatus",array(0,4))->get()->row();
                    ...
                    <div style='width:50px;' class="position-relative d-flex justify-content-center">
                        ...
                        <div style='margin-left:20px;'
                            ...
                            if($list->status==2 | $list->status==4 | $list->status==0){
                                    $time = $this->db->select("checkoutdate")->from("booked_info")->where("checkindate<=",date("Y-m-d H:i:s"))->where("checkoutdate>=",date("d-m-Y H:i:s"))->where("FIND_IN_SET(".$list->roomno.",room_no)<>",0)->where_in("bookingstatus",array(0,4))->get()->row();
                                    if(!empty($time->checkoutdate)){
                                    $diff = strtotime($time->checkoutdate) - strtotime(date("d-m-Y H:i:s"));
                            ...
                            <?php }
                                    ?>
                            <input type="hidden" id="date_time" value="<?php echo date("d-m-Y"); ?>">
